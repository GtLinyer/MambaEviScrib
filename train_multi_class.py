...
parser.add_argument('--root_path', type=str,
                    default='<your dataset path>/CardiacUDA', help='Name of Experiment')
parser.add_argument('--exp', type=str,
                    default='CardiacUDA/MambaEviScrib', help='experiment_name')
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            label4 = (label_batch == 4).unsqueeze(1)
            mask = 1 - (label_batch == 5).int().unsqueeze(1)
            label_onehot = torch.cat([label0, label1, label2, label3, label4], dim=1).int()
            # ====== start ======
            ...
            evidence_cnn_1 = torch.exp(torch.tanh(outputs_cnn) / 0.2)
            ...
            evidence_mamba_1 = torch.exp(torch.tanh(outputs_mamba) / 0.2)
