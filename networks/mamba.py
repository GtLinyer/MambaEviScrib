...
class VSSM(nn.Module):
    def __init__(self, patch_size=4, in_chans=1, num_classes=4, depths=[2, 2, 2, 2],
    ...
    def forward(self, x):
        ...
        print(x.shape, x_downsample[0].shape, x_downsample[1].shape)
...
if __name__ == "__main__":
    ...
    model = VSSM().cuda()
    in_data = torch.randn(4, 1, 256, 256).cuda()
    out = model(in_data)
    # print(out.shape)
